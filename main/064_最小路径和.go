...
func minPathSum(grid [][]int) int {
	/*
		...
		具体分为以下 4 种情况：
		当左边和上边都不是矩阵边界时： 即当i != 0 j != 0 时dp[i][j] = min(dp[i - 1][j], dp[i][j - 1]) + grid[i][j]dp[i][j]=min(dp[i−1][j],dp[i][j−1])+grid[i][j] ；
		当只有左边是矩阵边界时： 只能从上面来，即当i = 0, j != 0 i=0, dp[i][j] = dp[i][j - 1] + grid[i][j]dp[i][j]=dp[i][j−1]+grid[i][j] ；
		当只有上边是矩阵边界时： 只能从左面来，即当i != 0, j = 0 dp[i][j] = dp[i - 1][j] + grid[i][j]dp[i][j]=dp[i−1][j]+grid[i][j] ；
