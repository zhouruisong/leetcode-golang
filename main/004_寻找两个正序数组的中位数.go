...
func findMedianSortedArrays2(nums1 []int, nums2 []int) float64 {
	...
	/*
	   实现三元表达式的功能
	*/
	//threeFunc := func(condition bool, trueVal int, falseVal int32) int {
	//	if condition {
	//		return trueVal
	//	} else {
	//		return int(falseVal)
	...
	//}
	//
	//nums1LeftMax := threeFunc(i > 0, nums1[i-1], math.MinInt32)
	//nums1RightMin := threeFunc(i != m, nums1[i], math.MaxInt32)
	//nums2LeftMax := threeFunc(j > 0, nums2[j-1], math.MinInt32)
	//nums2RightMin := threeFunc(j != n, nums2[j], math.MaxInt32)
	//if ((m + n) % 2) == 1 {
